...
// ★2026-07-20(ユーザー指摘「生成中の秒数が止まる」対応): js/carving.jsの
// carveUnifiedRegions()内(visual_hull+accessories段、パイプライン中で
// 最も重い同期処理)にもtick()での小休止を挟めるよう、P3D経由で公開する
// (carving.jsはpipeline.jsより先に読み込まれるが、実際に呼ばれるのは
// ページ読み込み完了後なので問題ない)。
P3D.tick = tick;
...
async function runCarvingStages(state, report){
  ...
  // marching cubes直前にfieldへボクセル単位の3軸box blurをかけ、階段状の
  // ガクガクの発生源そのものを彫刻段階で削る(js/carving.jsのblurField3D
  // 参照)。強さ1〜10をボクセル半径1〜4へ写像する。
  var fieldSoftenRadius = (gp.field_soften_enable && gp.field_soften_strength>0)
    ? Math.max(1, Math.round(gp.field_soften_strength/2.5)) : 0;
  // ★2026-07-19追加(ユーザー指摘「輪郭ぼかしの痩せは太らせる機能ではきれいに
  // 戻らないのでは」対応): ぼかし半径に比例して等値面しきい値を0より下げ、
  // 同じフィールド空間内でぼかしによる痩せを打ち消す(js/carving.jsの
  // carveUnifiedRegions参照)。定数は実測(実サンプル)でぼかし単体の正味の
  // サイズ変化がほぼ0になるよう調整した値。
  var FIELD_SOFTEN_ISO_SHIFT_PER_RADIUS = -0.06;
  var fieldSoftenIsoShift = fieldSoftenRadius>0 ? fieldSoftenRadius*FIELD_SOFTEN_ISO_SHIFT_PER_RADIUS : 0;
  var unified = await P3D.carveUnifiedRegions(regions, sharedGrid, 0.001, partGapCloseDist, fieldSoftenRadius, fieldSoftenIsoShift);
...
function decimateStage(inter, gp){
  ...
  if(gp.decimate){
    ...
    // computeEdgeCollapseCost参照): 間引きアルゴリズム自体に、彫刻直後の
    // 元の密度を固定の物差しとしたハード禁止ルールを持たせた。事後補正
    // (座標を動かして直す)方式は、正常な髪の毛先まで動かしてしまい、かつ
    // 直しきれないスパイクが残る実測結果が出たため撤回した経緯がある。
    var spikeGuardRatio = gp.decimate_spike_guard ? (gp.decimate_spike_guard_ratio||0) : 0;
    var d=P3D.decimateMesh(V,F,gp.decimate_strength,owner,spikeGuardRatio);
...
function meshFinishStage(decimated, gp, pivots, partsMeta){
  ...
  // ★2026-07-19改訂(ユーザー指摘「丸み補正は太らせる機能に変える、柔らかさ/
  // ガクガク解消は別の代替案(field_soften_*)に任せる」対応):
  // mesh上のLaplacian緩和で「痩せた分を法線方向へ戻す」設計は、実サンプル
  // (56,837頂点)で反復数十回では誤差レベルの変化しか出ないと実測で判明し
  // 撤回した(js/carving.jsのblurField3D、runCarvingStages側のfield_soften_*
  // 参照。ガクガクの解消はそちらが担当)。puni_*は素直に、頂点を法線方向へ
  // 一様にオフセットして体型を太らせるだけのシンプルな処理にした。
  if(gp.puni_enable && gp.puni_strength>0){
    var puniT = Math.max(0, Math.min(1, gp.puni_strength/10));
    var PUNI_MAX_INFLATE = 0.05; // model単位。strength最大時の法線方向オフセット量
    V = P3D.inflateMeshAlongNormals(V, N, PUNI_MAX_INFLATE*puniT);
    N = P3D.computeVertexNormalsOnly(V, F);
  }
  ...
  var split=P3D.splitMeshByOwner(V, F, owner, ownerIds, {N:N});
...
async function finishFromIntermediate(inter, opts, onProgress){
  ...
  var decSig = sig({d:gp.decimate, ds:gp.decimate_strength,
    sg:gp.decimate_spike_guard, sgr:gp.decimate_spike_guard_ratio});
  var decimated;
  ...
  var meshSig = decSig+"|"+sig({si:gp.smooth_iters, rsw:gp.rigid_soft_width, pe:gp.puni_enable, ps:gp.puni_strength});
  var meshFinished;
