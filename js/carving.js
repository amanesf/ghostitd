...
// ★2026-07-20追加(間引きスパイク対策、js/vendor/SimplifyModifier.jsの
// computeEdgeCollapseCost参照): spikeGuardRatio(省略可、0/未指定で無効)は
// コラプスの副作用で生まれる新しい辺が、彫刻直後の元の近傍スケールの何倍
// までなら許すかの上限。超える場合はそのコラプス自体を選択肢から完全に
// 除外する(ハード禁止、コストへの加算ではない)。彫刻結果(輪郭)は一切
// 変更しない。
function simplifyModifierDecimate(V, F, maxCost, owner, spikeGuardRatio){
  ...
  var simplified = modifier.modify(geo, safeCount, ownerWeights, maxCost, spikeGuardRatio);
...
function decimateMesh(V, F, strength, owner, spikeGuardRatio){
  ...
  if(typeof THREE !== "undefined" && THREE.SimplifyModifier){
    try{
      ...
      return simplifyModifierDecimate(V, F, maxCost, owner, spikeGuardRatio);
    }catch(e){
...
// ★2026-07-18追加(「ぷにぷに」丸み補正、法線方向へのメッシュ膨張用):
// computeNormalsFixWindingは巻き補正(連結成分ごとの符号付き体積判定)まで
// 込みで重いが、inflateMeshAlongNormals後の頂点法線再計算は頂点座標を
// 法線方向へ小さくずらすだけで三角形の表裏(巻き)は変わらないため、
// 巻き補正を省いた軽量な頂点法線計算だけで十分。Fは既に巻き補正済みの
// ものをそのまま使う。
function computeVertexNormalsOnly(V, F){
  var n=V.length/3, nf=F.length/3;
  var Nv=new Float32Array(n*3);
  for(var f=0; f<nf; f++){
    ...
    var ax=V[ia*3],ay=V[ia*3+1],az=V[ia*3+2];
    var bx=V[ib*3],by=V[ib*3+1],bz=V[ib*3+2];
    var cxp=V[ic*3],cyp=V[ic*3+1],czp=V[ic*3+2];
    var ux=bx-ax, uy=by-ay, uz=bz-az;
    var wx=cxp-ax, wy=cyp-ay, wz=czp-az;
...
P3D.computeVertexNormalsOnly = computeVertexNormalsOnly;

// 各頂点をその頂点法線方向にamount(model単位)だけ一様にオフセットする
// (「ぷにぷに」丸み補正の膨張ステップ)。凹み(脇・関節の折れ目等)は法線が
// 互いに逆を向く面同士の間にあるため、外側へ押し出すほど凹みが浅くなる
// 方向に働き自己交差しにくい。逆に薄い板状の造形(前髪の房、マフラーの端等)
// は表裏の法線がほぼ正反対なので、両面が互いから遠ざかる方向に厚みが増す
// だけで交差しない。
function inflateMeshAlongNormals(V, N, amount){
  var n=V.length/3;
  var out=new Float32Array(V.length);
  for(var i=0;i<n;i++){
    out[i*3]=V[i*3]+N[i*3]*amount;
    out[i*3+1]=V[i*3+1]+N[i*3+1]*amount;
    out[i*3+2]=V[i*3+2]+N[i*3+2]*amount;
  }
  return out;
}
P3D.inflateMeshAlongNormals = inflateMeshAlongNormals;
...
function carveRegion(opts){
  ...
  var rowSmooth=!!opts.rowSmooth, rowSmoothStrength=opts.rowSmoothStrength||0;
  ...
  function buildWidthTracks(){
    ...
    // ★2026-07-18追加(row_smooth設定、ユーザー指摘「顔に横線が入る」対応):
    // trackLabelsで連結成分ごとに分けたtrack(前髪の房等、互いに独立した
    // ストランド)ごとに、rows配列に沿って(=iy=行の並び順に沿って)cx/hwへ
    // 単純移動平均をかける。trackをまたいで平均すると別の房同士が混ざって
    // 位置がずれるため、必ずtrack単位(tr3.rows/cx/hw)で閉じて行う。
    // ★2026-07-18バグ修正(ユーザー報告「行の幅平滑化で行が消滅する」):
    // 1つのtrack(連結成分)でも、股が分かれた脚(腰でつながって1連結成分だが
    // 各行は左右2本)や根元でつながった前髪の房のように、同じ行に複数
    // セグメントを持つことがある。tr3.rows/cx/hwはそのセグメントを行の並び順
    // (かつ同じ行の中ではx昇順)にフラットに積んだ配列で、上記のようなtrackは
    // [左脚,右脚,左脚,右脚,...]のように交互に並ぶ。旧実装は「配列インデックス
    // ±win」を単純に平均していたため、左脚の中心が右脚の値と混ざって
    // 中心線側へ引っ張られ、平滑化後のcxが実際のシルエットより内側にずれて
    // いた。半幅(hw)自体はゼロにならないため彫刻自体は消えないが、真の外周
    // (cx±hw)が塗られなくなる分だけ輪郭の外側が彫刻から漏れ、「行(輪郭の
    // 一部)が消える」ように見えていた。
    // 修正: 同じ行内のセグメントをx昇順の「列」とみなし(1行1セグメントの
    // track=列0のみ、という従来ケースはそのまま)、列ごとに独立して
    // (=同じ枝同士でだけ)window平均する。列をまたいだ平均は行わない。
    if(rowSmooth && rowSmoothStrength>0){
      var win=Math.max(1, Math.round(rowSmoothStrength));
      tracksByLabel.forEach(function(tr3){
        var m=tr3.rows.length;
        if(m<2) return;
        // 同じiyが連続する範囲を1つの行グループとする(buildWidthTracksの
        // 積み方により、同じ行のセグメントは必ず配列上で連続している)。
        var rowGroups=[]; var curIy=null, curGroup=null;
        for(var gi=0; gi<m; gi++){
          if(tr3.rows[gi]!==curIy){ curIy=tr3.rows[gi]; curGroup=[]; rowGroups.push(curGroup); }
          curGroup.push(gi);
        }
        var maxCols=0;
        for(var gi2=0; gi2<rowGroups.length; gi2++) maxCols=Math.max(maxCols, rowGroups[gi2].length);
        var cxOut=new Float64Array(m), hwOut=new Float64Array(m);
        for(var col=0; col<maxCols; col++){
          // この列(col)を持つ行(rowGroups中の位置)だけを行の並び順で集める。
          var seq=[];
          for(var gi3=0; gi3<rowGroups.length; gi3++){ if(rowGroups[gi3].length>col) seq.push(gi3); }
          var sm=seq.length;
          for(var si2=0; si2<sm; si2++){
            var lo=Math.max(0,si2-win), hi=Math.min(sm-1,si2+win);
            var sc=0,sh=0,cnt=0;
            for(var j=lo;j<=hi;j++){
              var srcIdx=rowGroups[seq[j]][col];
              sc+=tr3.cx[srcIdx]; sh+=tr3.hw[srcIdx]; cnt++;
            }
            var dstIdx=rowGroups[seq[si2]][col];
            cxOut[dstIdx]=sc/cnt; hwOut[dstIdx]=sh/cnt;
          }
        }
        for(var i5=0;i5<m;i5++){ tr3.cx[i5]=cxOut[i5]; tr3.hw[i5]=hwOut[i5]; }
      });
    }
...
// ★2026-07-19追加(「ぷにぷに」丸み補正の代替案、ユーザー要望「彫刻前にSDF
// フィールドをぼかす方式を追加してほしい」対応): mesh上のLaplacian平滑化
// (js/carving.jsのlaplacianSmooth/gen_params.smooth_iters)は不規則な
// メッシュグラフ上を1ホップずつしか伝播しないため、実キャラクターメッシュ
// (数万頂点)くらい密だと反復数十回では誤差レベルの変化しか出ず、視認できる
// 効果には数百〜千反復が必要で非現実的(実測により判明、js/pipeline.jsの
// meshFinishStage参照)。対してfieldは規則格子(ボクセルグリッド)なので、
// 軸ごとに独立な1次元box blur(スライディングウィンドウ和、半径によらず
// O(要素数))を3軸へ順にかけるだけで、少ない計算量のまま広い実効半径の
// ぼかしを一度に効かせられる。marching cubes直前にこれをかけることで、
// 階段状のガクガクの発生源(ボクセル量子化そのもの)を削れる。
// 3軸(x,y,z)それぞれについて、その軸方向1次元のboxBlur(半径radius、
// スライディングウィンドウ和、範囲外はエッジ値を延長=clamp)をかける。
// field: Float32Array(ny*nx*nz)、レイアウトはcarveSdfField/marchingCubesと
// 同じ idx = iy*(nx*nz) + ix*nz + iz。
function blurFieldAxis(field, ny, nx, nz, axis, radius){
  ...
  if(axis==='z'){
    for(var iy=0; iy<ny; iy++){
      for(var ix=0; ix<nx; ix++){
        var base=iy*(nx*nz)+ix*nz;
        var sum=0;
        for(var k=-radius;k<=radius;k++) sum+=field[base+Math.max(0,Math.min(nz-1,k))];
        for(var iz=0; iz<nz; iz++){
          out[base+iz]=sum/win;
          var addIz=iz+radius+1, remIz=iz-radius;
          var addVal=field[base+Math.max(0,Math.min(nz-1,addIz))];
          var remVal=field[base+Math.max(0,Math.min(nz-1,remIz))];
          sum += addVal-remVal;
        }
      }
    }
    ...
    for(var iy2=0; iy2<ny; iy2++){
      for(var iz2=0; iz2<nz; iz2++){
        var base2=iy2*(nx*nz)+iz2;
        var sum2=0;
        for(var k2=-radius;k2<=radius;k2++) sum2+=field[base2+Math.max(0,Math.min(nx-1,k2))*nz];
        for(var ix2=0; ix2<nx; ix2++){
          out[base2+ix2*nz]=sum2/win;
          var addIx=ix2+radius+1, remIx=ix2-radius;
          var addVal2=field[base2+Math.max(0,Math.min(nx-1,addIx))*nz];
          var remVal2=field[base2+Math.max(0,Math.min(nx-1,remIx))*nz];
          sum2 += addVal2-remVal2;
        }
      }
    }
  }else{ // 'y'(呼び出し側は1ウィンドウ=streaming分割のy範囲を丸ごと渡す前提)
    for(var ix3=0; ix3<nx; ix3++){
      for(var iz3=0; iz3<nz; iz3++){
        var base3=ix3*nz+iz3;
        var sum3=0;
        for(var k3=-radius;k3<=radius;k3++) sum3+=field[base3+Math.max(0,Math.min(ny-1,k3))*(nx*nz)];
        for(var iy3=0; iy3<ny; iy3++){
          out[base3+iy3*(nx*nz)]=sum3/win;
          var addIy=iy3+radius+1, remIy=iy3-radius;
          var addVal3=field[base3+Math.max(0,Math.min(ny-1,addIy))*(nx*nz)];
          var remVal3=field[base3+Math.max(0,Math.min(ny-1,remIy))*(nx*nz)];
          sum3 += addVal3-remVal3;
        }
      }
    }
  }
...
// field(1ウィンドウ分)へx/y/zの順にbox blurをかけて返す(新規配列、inplace
// ではない)。radius=0で無効(元のfieldをそのまま返す)。
// ★streaming彫刻の制限: yのぼかしはこの関数に渡されたウィンドウ内で完結する
// (ウィンドウ境界の外側の行は参照しない、clamp扱い)。既定の粗い解像度設定
// では常にウィンドウ1個(carveUnifiedRegions内STREAMING_FIELD_BUDGET_BYTES
// 参照)でこの制限は顕在化しない。
function blurField3D(field, ny, nx, nz, radius){
  if(!(radius>0)) return field;
  var fx=blurFieldAxis(field, ny, nx, nz, 'x', radius);
  var fy=blurFieldAxis(fx, ny, nx, nz, 'y', radius);
  var fz=blurFieldAxis(fy, ny, nx, nz, 'z', radius);
  return fz;
}
P3D.blurField3D = blurField3D;
 ...
 * fieldSoftenRadius: 省略可(ボクセル単位の整数、0または未指定で無効)。
 *   marching cubes直前にfieldへ3軸box blurをかける半径(field_soften_*、
 *   blurField3D参照)。階段状のガクガクの発生源(ボクセル量子化)を彫刻段階で
 *   直接削る「丸み補正」の代替案。
...
// ★2026-07-20(ユーザー指摘「生成中の秒数が止まる」対応): このパイプライン中
// 最も重い同期処理で、生成中は#genSecondsのsetIntervalがブラウザに割り込む
// 隙が無いまま数秒〜数十秒ブロックしてしまい、経過秒数表示が止まって見えていた。
// パーツ(体+各アクセサリー)ごとの実測行スキャン・SDF書き込みは元々1パーツ
// ずつ独立して行える単位なので、その区切りごとにP3D.tick()で1度だけ
// イベントループに制御を返し、setIntervalやDOM再描画が割り込めるようにする
// (「なぜモジュール化していないか」と同じ理由で、彫刻アルゴリズム自体は
// 一切変えていない。async化+待ち点の追加のみ)。
async function carveUnifiedRegions(regions, sharedGrid, minFragFrac, partGapCloseDist, fieldSoftenRadius, fieldSoftenIsoShift){
  ...
  for(var pri=0; pri<regions.length; pri++){
    var rPre=regions[pri];
    precomputedRowsByOwner.set(rPre.ownerId, carveRegion(Object.assign({}, rPre.opts, {grid:sharedGrid, extentsOnly:true})));
    await P3D.tick();
  }
  ...
  for(var winStart=0; winStart<ny-1; winStart += (windowRows-1)){
    ...
    // ★2026-07-19追加(「ぷにぷに」丸み補正の代替案、field_soften_*):
    // marching cubes直前、このウィンドウのfieldが確定した最後の時点で
    // 3軸box blurをかける。ownerFieldは整数ID配列なのでぼかさない
    // (境界付近のowner判定が数ボクセル分あいまいになるのは許容する)。
    if(fieldSoftenRadius>0){
      field = blurField3D(field, winRows, nx, nz, fieldSoftenRadius);
    }

    // ★2026-07-19追加(ユーザー指摘「輪郭ぼかしで痩せた分を、太らせる機能(法線
    // 方向への一様メッシュオフセット)で戻すのは仕組みが違うのできれいに戻らない
    // のでは」対応): ぼかしによる痩せは等値面ベースの現象(box blurが
    // 境界付近の値を平均して0クロス位置を動かす)なので、メッシュ側で後から
    // 一様に押し出すより、marching cubesの等値面しきい値そのものを0より
    // 下げて抽出する方が同じフィールド空間内で完結する分、筋が良い(完全な
    // 復元ではないが、太らせるの一様メッシュオフセットより痩せ方の実態に
    // 近い形で戻る)。fieldSoftenIsoShift(負の値)はrunCarvingStages側で
    // fieldSoftenRadiusから算出する。
    var isoValue = fieldSoftenIsoShift || 0.0;
    var mc = P3D.marchingCubes(field, winRows, nx, nz, isoValue, ownerField, {
