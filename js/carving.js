...
// ★2026-07-19追加(スパイク根本原因調査の結論、js/vendor/SimplifyModifier.jsの
// absorbCount方式(decimate_spike_guard、一度実装して撤回済み)は「1頂点に
// 吸収が集中するケース」にしか効かず、腰まわりの裂け目のように「かたまり
// 全体が高曲率として保護され続けるケース」には無力だと実データで確認できた。
// 一方この関数(spikeSmoothSelectiveの再実装、2026-07-18に一度追加→撤回された
// ものと同じ考え方)はメッシュ空間の頂点座標に直接作用するため、行ごとに
// 独立した奥行き測定が生む「帯の本数が1→2→1のように短い区間で入れ替わる」
// といったトポロジー変化(帯同士を補間しようとしても対応が取れず不可能)を
// 気にする必要が無い。実データ検証: この関数を彫刻直後(間引き前)のメッシュに
// 適用してから間引きすると、間引き後にも裂け目が残らないことを確認した
// (間引き後に適用する旧方式は改善が緩やかだった。間引きが頂点・近傍情報を
// 削ってしまってからでは、Kホップ平均が参照できる「本来の面」の情報が
// 既に失われているため)。
...
// 辿った範囲(自分を除く全頂点)の平均を使う。実際の飛び出し(1行だけ帯が
// 分裂して迫り出す等)は「1点だけ尖っている」のではなく「隣接する複数の頂点が
// まとまって同じように迫り出している面」であり、1-ringだけの平均では塊の
// 内部同士を比べることになり差が出ず検出できない(実測)。塊の外側(迫り出して
// いない本来の面)まで届く広さの平均を基準にすることで、塊全体が本来の位置
// よりズレていること自体を検出できる。
