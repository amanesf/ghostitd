...
var DEFAULT_GEN_PARAMS = {
  ...
  // ★2026-07-19追加(スパイク調査、ユーザー承認): SimplifyModifierの間引きは
  // 曲率の高い頂点(段差の角)を保護し、その代わり周囲の頂点から優先的に
  // 間引く。彫刻直後は複数頂点にまたがるなだらかな段差(肩・太もも・
  // スカート裾・頭頂等、front/side/back画像の実測に基づく本物の形状)が、
  // 間引きで緩衝頂点だけ失われて1頂点への吸収が集中すると、頂点の座標自体は
  // 動かないまま鋭い突起(スパイク)に見えるようになる不具合があった(実データ
  // で検証、「近傍重心からの突出量/近傍辺長」が間引き前後で10〜20倍化する
  // ことを確認済み)。彫刻結果(輪郭)自体は一切変更せず、間引きが1頂点に
  // 偏らないよう分散させるだけの機能のため既定ONにした
  // (js/vendor/SimplifyModifier.jsのabsorbCount/_spikeGuardCap参照)。
  decimate_spike_guard: true, decimate_spike_guard_max: 8,
