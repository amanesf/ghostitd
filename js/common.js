...
var DEFAULT_GEN_PARAMS = {
  ...
  // ★2026-07-19追加(スパイク根本原因調査、ユーザー承認): 行ごとに独立した
  // 奥行き測定の都合上、パーツの輪郭が実際に途切れる/先細りする「末端」
  // (裾・頭頂・腕/脚の先等)では、本物の細部(布のめくれ等)がごく少ない行数に
  // 圧縮され、彫刻結果がなだらかに繋がらないことがある(js/carving.jsの
  // spikeSmoothSelective参照)。近傍平均からのズレがspike_smooth_threshold
  // (model単位)を超えた頂点だけを、超えた分だけ引っ込める。対象パーツは
  // body_spike_smooth(体、全身一律)とアクセサリー個別のspikeSmooth(既定0=
  // 対象外)で選ぶ。★間引き(decimateStage)より前、彫刻直後の生メッシュに
  // 適用する(間引き後に適用すると、間引きで既に失われた近傍頂点の情報を
  // 参照できず改善が弱いことを実データで確認したため)。既定は全パーツ0
  // (無効)で、必要なパーツだけ手動でONにする。
