...
var DEFAULT_GEN_PARAMS = {
  ...
  // computeEdgeCollapseCost参照): エッジコラプスの副作用で生まれる辺が、
  // 彫刻直後(コラプスが1回も起きていない時点)の元の近傍密度に対して
  // decimate_spike_guard_ratio倍を超える場合、そのコラプス自体をハード禁止
  // する(コストへの加算ではなく選択肢から完全除外)。間引きが進んでも
  // 判定基準そのものは動かない固定値を使うため、コラプスが何回連鎖しても
  // すり抜けない。彫刻結果(輪郭)・彫刻直後の生メッシュには一切影響しない。
  // ★2026-07-19: ユーザー指摘によりデフォルトOFFに変更(実サンプルでは発動
  // 頻度が低く、常時ONにするほどの効果を体感できなかったため)。必要な時だけ
  // 個別に有効化する。
  decimate_spike_guard: false, decimate_spike_guard_ratio: 3,
  ...
  // ★2026-07-17(設定タブ整理): track_win/track_gap_close_px/depth_gap_close_px
  // は廃止した。前面∪背面の合成(buildCombinedFrontRaster)で大半のケースは
  // 既に解決しており、実サンプルで実際に隙間が埋まる場面を検証したところ
  // (色分けマップの実データ上で)効果を確認できなかった(ユーザー指摘)。
  // track_winは既定値1(=1行の中央値=無効)のまま長らく使われていなかった。
  ...
  // ★2026-07-17(設定タブ整理、ユーザー指摘「輪郭を正とすべき」): 目窩
  // (face_sculpt/eye_socket_depth/eye_socket_radius_x/eye_socket_radius_y)は
  // front/back/sideどのシルエット輪郭にも現れない、ランドマーク位置からの
  // 完全な合成形状だった。輪郭(実測データ)だけを信じる方針と矛盾するため
  // 機能ごと廃止した(js/carving.jsのapplyEyeSocketRecess参照、削除済み)。
  // ★2026-07-17(設定タブ整理): bone_bridge(遮蔽で欠けたパーツの橋渡し)は
  // 廃止した。実サンプル(ツインテールが腕に隠れるケース)で実際に検証した
  // ところ、前面∪背面の合成(buildCombinedFrontRaster)で既に隙間なく繋がって
  // おり、bone_bridgeが対象とする「前面∪背面でも解決しない隙間」という
  // ケース自体を実データで確認できなかった(ユーザー指摘)。
  // ★2026-07-14追加(ユーザー要望「手動マップではなく、モデル生成時に隙間を
  // 埋める機能がほしい。設定した距離をパーツ間だけ塞ぐ」): bone_bridgeの
  // ボーン共有+行単位の粗い近似とは独立に、ボクセルグリッド上の本物の
  // 3次元距離だけで異なるパーツ間の隙間を塞ぐ(js/carving.jsの
  // closeInterPartGaps参照)。まだどのパーツも領域を持たない空ボクセルの
  // うち、異なる2パーツ以上の表面から同時にpart_gap_close_dist以内に
  // 届いているボクセルだけを埋める(同一パーツ内の凹みや、1パーツだけが
  // 近い外周には影響しない)。粗い近似を経由しないため、bone_bridgeで
  // 繰り返し見つかった「実際には近くないのに隣接と誤判定する」不具合が
  // 原理的に起きにくい。
  part_gap_close: false, part_gap_close_dist: 0.01,
  // ★2026-07-18追加(ユーザー指摘「顔に横線が入る、平滑化では狙い撃ちで
  // 消せない」対応): buildWidthTracks(js/carving.js)は行(iy、Y方向の高さ)
  // ごとにfront/back画像から独立にシルエット幅(cx/hw)を読み取るため、
  // 隣接行同士でわずかな誤差があると彫刻結果に階段状の段差(横線)が
  // そのまま残る。全体を均すLaplacian平滑化(smooth_iters)はこの上下方向
  // だけのノイズを狙い撃ちできず、強くかけると輪郭全体がぼやける。
  // そこで彫刻前(幅を読み取った直後)にrow_smooth_strength行分の移動平均を
  // iy方向にかけ、段差の発生自体を抑える(js/carving.jsのsmoothRowTrack参照)。
  row_smooth: false, row_smooth_strength: 2,
  // ★2026-07-18追加→2026-07-19改訂(ユーザー要望「ガクガクして可愛くない、
  // 丸み補正(ぷにぷに感)がほしい」→ユーザー指摘「まだ硬い、どこまで強さを
  // 上げてもガクガクが取れない」→実測(実サンプル56,837頂点)でmesh上の単純
  // Laplacian緩和は反復数十回程度では誤差レベルの変化しか出ず、視認できる
  // 効果を出すには数百〜千反復オーダーが必要と判明し非現実的と分かったため
  // 撤回): 「柔らかさ・ガクガク解消」は彫刻段階(marching cubes前のSDF
  // フィールドをぼかす、field_soften_*参照)に役割を移し、puni_*は素直な
  // 「法線方向への一様膨張(体型を太らせる)」専任にした
  // (js/pipeline.jsのmeshFinishStage、js/carving.jsのinflateMeshAlongNormals
  // 参照)。
  puni_enable: false, puni_strength: 3,
  // ★2026-07-19追加(ユーザー要望「丸み補正の代替案として、彫刻前にSDF
  // フィールドをぼかす方式を追加してほしい」対応): marching cubes直前の
  // ボクセルSDFフィールド(規則格子)に分離可能なbox blurをかけ、階段状の
  // ガクガクの発生源そのものを削る、彫刻段階(Tier2/3、要再生成)の丸み補正。
  // mesh上のLaplacian(js/carving.jsのlaplacianSmooth/smooth_iters)と違い
  // 規則格子上のブラーなので反復回数を増やさずに広い半径を一度に効かせられる
  // (js/carving.jsのblurField3D、js/pipeline.jsのrunCarvingStages参照)。
  // ボケた分だけ輪郭が痩せて見える場合はpuni_strength(太らせる)と併用する
  // 想定(この関数自体はサイズを戻す補正をしない、独立した2本のスライダー)。
  field_soften_enable: false, field_soften_strength: 3,
};
