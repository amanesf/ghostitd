...
var DEFAULT_GEN_PARAMS = {
  ...
  // (js/carving.jsのcarveSdfField/carveUnifiedRegions参照)。
  // ★2026-07-13追加(ユーザー指摘「ツインテールの付け根で隙間が空く」対応):
  // 遮蔽(例: ツインテールが後ろ髪の一部を覆い隠す)によってシルエットその
  // ものが欠落したパーツ同士の隙間は、単純な境界ブレンドでは埋まらない
  // (=そもそも競合するボクセルが存在しない)。bones(付け根の骨、例えば
  ...
  // applyBoneBridgeFill、js/pipeline.jsのrunCarvingStages参照)。
  // ★2026-07-14(ユーザー指摘「箱型になって意味不明」対応): 当初の実装は
  // 橋渡し範囲を2パーツの外縁の和集合(union)で計算しており、前髪⇔顔の
  // ように境界が広く連続したペアでは行ごとに断面丸ごとに近い範囲を塗る
  // 巨大な箱になっていた。範囲計算を「隙間/重なりの近傍だけ」に絞り、
  // さらに幅・奥行きの両軸とも"重なり"の行(体のような大きいパーツと、
  // その上に乗る/被さるアクセサリーのバウンディング範囲がたまたま両軸とも
  // 大きく重なっているだけの偽陽性が多かった)を橋渡し対象から除外する
  // よう修正した。ツインテール付け根・前髪⇔後ろ髪いずれも意図通りの
  // 細い継ぎ目になることを確認済みだが、体はbonesを持たず常に全アクセサリー
  // の橋渡し候補になる点など、まだ誤爆の余地が完全には無くなっていないため、
  // 既定はOFFとし使いたい場合は設定タブから有効化する実験的機能とする
  // (同時に、同様の目的だったowner_blend機能は実測で効果がほぼ無いことが
  // 分かったため機能ごと削除した)。
  bone_bridge: false,
