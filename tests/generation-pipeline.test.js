...
// ★2026-07-18(続き): buildWidthTracksに、行内の主要セグメントより極端に
// 小さい(15%未満)セグメントを無視するフィルタを追加。奥行き(hdByRow)は
// 1行につき1値しか持てないのに幅(segs)は複数の塊を持てるという非対称性が
// あり、体の脚が分かれ始める境目のような本物の細い切れ込みがある行では、
// 本体から数px離れた極小の断片が独立したセグメントとして拾われ、
// carveSdfField側で全セグメントに同じ行の奥行き(本体の値)がそのまま
// 適用されてしまっていた(実測・45度レンダリングで確認したスカート裾の
...
// ★2026-07-17: サンプル2(landmarks_ai_2_embedded.json)をAI生成の新データに
// 差し替えたため(ユーザー指摘「landmarks_ai_2.jsonをジェネレータのデフォルト
// にして、サンプルも置き換えて」)、彫刻の入力そのものが変わりハッシュを更新した。
// ★2026-07-17(再更新): normalizeSeamSideDefaults()(側面画像利用の既定値を
// 「使わない」に強制していた処理。JSON再読み込みのたびにユーザーが選んだ
// 「使う」を「使わない」へ戻してしまう不具合の原因だった)を廃止し、
// atlas.js本来の既定「未指定=側面画像を使う」に統一した(ユーザー指摘
// 「側面画像利用の設定が戻っちゃう、あとデフォルトオンにしたい」)。サンプルの
// seam_no_sideは空なので、この変更で全パーツが側面画像を使うようになり
// テクスチャ焼き込み結果(=GLB)が変わるため、ハッシュを再度更新した。
// ★2026-07-20(ユーザー報告「脇の下にありえないスパイクが出る」原因調査・
// 修正): 彫刻結果自体(rawV/rawF)は正常(実測: 隣接エッジ長の局所比が全体で
// 高々1.4倍)だが、間引き(js/vendor/SimplifyModifier.jsのエッジコラプス)が
// 曲率の高い頂点を保護し周囲だけを吸収する設計のため、座標は動いていない
// のに近傍密度だけが局所的に失われ、鋭い突起(スパイク)に見える不具合が
// あった(実測: 局所比が最大26.6倍まで悪化するケースを確認)。事後的に
// 頂点を動かして補正する方式(髪の毛先等の正常なディテールまで動かし、かつ
// 直しきれないスパイクが残る実測結果が出て撤回)ではなく、間引きアルゴリズム
// 自体に「コラプスの副作用で生まれる辺が、彫刻直後(コラプスが1回も起きて
// いない時点)の元の近傍密度の何倍まで伸びてよいか」のハード禁止ルールを
// 追加した(js/vendor/SimplifyModifier.jsのcomputeEdgeCollapseCost、既定
// decimate_spike_guard_ratio=3、超過するコラプスは選択肢から完全除外)。
// 間引きが何回連鎖しても判定基準(元の密度)自体は動かないため、小さな
// 伸びが少しずつ積み重なる「じわじわ型」もすり抜けない。該当箇所は間引きが
// 弱まり密なまま残るため、body-only/with-accessoryとも頂点数が増え(=ファイル
// サイズが増加)、ハッシュを更新した(側面画像利用の既定値変更と合わせて
// 両方の変更を反映した値に更新)。
const EXPECTED_BODY_ONLY = {
  byteLength: 4794800,
  sha256: "88686ce1f17b913253baf1ea6591ce1c0bb8bebb68d78d727ce7b9645dfb9005",
};
...
// ★2026-07-17: 上のEXPECTED_BODY_ONLYコメント(サンプル2差し替え、側面画像利用の
// 既定値をONに変更)参照。
// ★2026-07-20: 上のEXPECTED_BODY_ONLYコメント(間引きスパイクのハード禁止
// ルール追加)と同じ変更により、with-accessory側も彫刻結果が変わり
// ハッシュを更新した。
const EXPECTED_WITH_ACCESSORY = {
  byteLength: 11699688,
  sha256: "7fa8b95fe15bcaf25111376eeef70f41ef7c95225c512816864079000b08a9ea",
};
...
async function generateAndExportGlb(server, browser, bodyOnly) {
  ...
  if (bodyOnly) {
    // 既定サンプル(SAMPLES[0]、サンプル3)のJSONからaccessoriesだけを空にした
    // 一時JSONを作り、「JSON読み込み」機能経由で読み込ませる(画像は先に
    // 読み込み済みのものを使う。このJSONにも画像dataUrlは含まれるが、既に
    // pointsがある=編集画面に入ってからの読み込みなのでapplyLoadedJson()の
    // 差し替え経路のみが働き、画像dataUrl自体は参照されない)。
    const srcJson = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, "landmarks_ai_3_embedded.json"), "utf8"));
    srcJson.accessories = [];
    const tmpPath = path.join(os.tmpdir(), "landmarks_ai_3_body_only_" + Date.now() + ".json");
