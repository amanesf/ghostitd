...
async function testEditorTabsAndOverlays(server, browser) {
  ...
  // ★2026-07-20(設定タブUI見直し): 区分はdetails.accgroupのアコーディオンから
  // [data-paramgroup]の横並びタブに変わった。タブ切替でも例外が出ないことを
  // 確認する。
  await page.click('.tabbtn[data-tab="params"]');
  await page.waitForTimeout(150);
  const firstGroupTab = await page.$("[data-paramgroup]");
  assert.ok(firstGroupTab, "params group tab button should exist");
...
// 分割された。
// ★2026-07-20(設定タブUI見直し): 区分(6個)はdetails.accgroupの縦積み
// アコーディオンから[data-paramgroup]の横並びタブに、さらに区分内の話題は
// [data-paramsubgroup]のサブサブタブに変わった(詳細はlandmark_tool.html
// のPARAM_SUBGROUPSコメント参照)。継ぎ目角度パネル(BONE_GROUPSアコーディオン)
// 自体は「側面画像の利用」サブサブタブの中に従来通りdetails.accgroupのまま
// 残っている。
async function testModelAdjustAndSeamPanel(server, browser) {
  ...
  const finenessGroup = await page.$('[data-paramgroup="モデルの細かさ"]');
  assert.ok(finenessGroup, "モデルの細かさ group tab should exist");
  await finenessGroup.click();
  await page.waitForTimeout(150);
  const decimateSubtab = await page.$('[data-paramsubgroup="間引き"]');
  assert.ok(decimateSubtab, "間引き subtab should exist under モデルの細かさ");
  await decimateSubtab.click();
  await page.waitForTimeout(150);
  ...
  const pasteGroup = await page.$('[data-paramgroup="モデルへの画像貼り付け"]');
  assert.ok(pasteGroup, "モデルへの画像貼り付け group tab should exist");
  await pasteGroup.click();
  await page.waitForTimeout(150);
  const seamSideSubtab = await page.$('[data-paramsubgroup="側面画像の利用"]');
  assert.ok(seamSideSubtab, "側面画像の利用 subtab should exist under モデルへの画像貼り付け");
  await seamSideSubtab.click();
  await page.waitForTimeout(150);
...
// ★2026-07-20(ユーザー指摘「ボーン選択はプルダウンに」対応): rigidは単一
// <select>、softはドロップダウン+「追加」ボタンでタグのように積み上げる方式
// (buildAccessoryBoneItemHtml/wireAccessoryBoneEvents)。追加・重複防止・削除の
// 一連の操作が例外なく動くことを確認する。
async function testAccessoryBoneDropdown(server, browser) {
  const { page, errors } = await openPage(browser, server.url + "/landmark_tool.html");
  await page.click("#modeSampleBtn b");
  await page.waitForFunction(
    () => !document.getElementById("editor").classList.contains("hidden"),
  ...
  const rigidSelect = await page.$('.automaskBoneSelectRigid[data-i="0"]');
  assert.ok(rigidSelect, "rigid bone select should be present for the first mask accessory");

  // ★モードを切り替えても既存のbones配列は引き継がれる仕様(buildAccessoryBoneItemHtml
  // 参照、切り詰めはsoft→rigidの1本化時のみ)なので、切り替え前に一旦空にしてから
  // soft側の追加・重複防止・削除だけを検証する。
  await page.selectOption('.automaskBoneSelectRigid[data-i="0"]', "");
  await page.waitForTimeout(100);

  await page.click('.automaskModeBtn[data-i="0"][data-mode="soft"]');
  await page.waitForTimeout(150);
  const softSelect = await page.$('.automaskBoneSelectSoft[data-i="0"]');
  assert.ok(softSelect, "soft bone select should appear after switching to soft mode");
  await page.selectOption('.automaskBoneSelectSoft[data-i="0"]', "head");
  await page.click('.automaskBoneAdd[data-i="0"]');
  await page.waitForTimeout(150);
  await page.selectOption('.automaskBoneSelectSoft[data-i="0"]', "head");
  await page.click('.automaskBoneAdd[data-i="0"]'); // 重複追加は無視されるはず
  await page.waitForTimeout(150);
  let bones = await page.$$eval(".automaskBoneRemove", (els) => els.map((e) => e.dataset.bone));
  assert.deepStrictEqual(bones, ["head"], "adding the same bone twice should not create a duplicate row");

  await page.click('.automaskBoneRemove[data-i="0"][data-bone="head"]');
  await page.waitForTimeout(150);
  bones = await page.$$eval(".automaskBoneRemove", (els) => els.map((e) => e.dataset.bone));
  assert.deepStrictEqual(bones, [], "removing the only added bone should leave the list empty");
...
async function run() {
  ...
  try {
    ...
    await testAccessoryBoneDropdown(server, browser);
  } finally {
    await browser.close();
    await server.close();
  }
